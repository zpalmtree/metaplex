...
program
    ...
    .action(async (_, cmd) => {
        ...
        const antiRug =
            ...
            selfDestructDateActual
                ? {
                      ...
                      tokenRequirement: new anchor.BN(
                          antiRugTokenRequirementNumber,
                      ),
        ...
        const phaseOneStartDateActual =
            (phaseOneStartDate ? Date.parse(phaseOneStartDate) : Date.now()) /
            1000;
        const phaseOneEndDateActual =
            (phaseOneEndDate
                ? Date.parse(phaseOneEndDate)
        ...
        if (!treasuryMint) {
            ...
            priceRangeEndNumber = Math.ceil(
                priceRangeEndNumber * LAMPORTS_PER_SOL,
            );
...
program
    ...
    .action(async (_, cmd) => {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            try {
                fairLaunchTicketObj =
                    await anchorProgram.account.fairLaunchTicket.fetch(
                        fairLaunchTicket,
                    );
                break;
            } catch (e) {
...
program
    ...
    .action(async (_, cmd) => {
        ...
        const mantissa = 10 ** mintInfo.decimals;
        const assocToken =
            await anchorProgram.provider.connection.getAccountInfo(token);
        if (!assocToken) {
            instructions.push(
...
async function adjustTicket({
    ...
    instructions.push(
        await anchorProgram.instruction.adjustTicket(
            new anchor.BN(amountNumber),
            {
                accounts: {
                    fairLaunchTicket,
...
program
    ...
    .action(async (_, cmd) => {
        ...
        const [participationTokenAccount, tokenBump] =
...
program
    ...
    .action(async (_, cmd) => {
        ...
        const ticketData: { key: anchor.web3.PublicKey; model: any }[][] =
            await Promise.all(
                chunks(
                    Array.from(Array(ticketsFlattened.length).keys()),
                    1000,
                ).map(async allIndexesInSlice => {
                    let states = [];
                    for (let i = 0; i < allIndexesInSlice.length; i += 100) {
                        console.log(
                                ...
                                const myByte =
                                    fairLaunchLotteryBitmapObj.data[
                                        FAIR_LAUNCH_LOTTERY_SIZE +
                                            Math.floor(
                                                ticket.model.seq.toNumber() / 8,
                                            )
                                    ];

                                const positionFromRight =
                                    7 - (ticket.model.seq.toNumber() % 8);
...
async function getParticipationNft({
    ...
    if (
        ...
        fairLaunchTicketObj.seq.toNumber() %
            fairLaunchObj.participationModulo ==
            0
    ) {
...
program
    ...
    .action(async (_, cmd) => {
        ...
        console.log(
            `Added ${amountNumber} tokens to ${tokenAccount.toBase58()}`,
        );
    });
...
program
    ...
    .action(async (_, cmd) => {
        ...
        const [fairLaunchLotteryBitmap, bump] =
            await getFairLaunchLotteryBitmap(
                //@ts-ignore
                fairLaunchObj.tokenMint,
            );
...
program
    ...
    .option('-f, --fair-launch <string>', 'fair launch id')
    .option(
        '-b, --fair-launch-ticket-buyer <string>',
        'fair launch ticket buyer',
    )
    .action(async (options, cmd) => {
    ...
    .action(async (options, cmd) => {
        ...
        for (let i = 0; i < buyersFlattened.length; i++) {
            ...
            const myByte =
                fairLaunchLotteryBitmapObj.data[
                    FAIR_LAUNCH_LOTTERY_SIZE +
                        Math.floor(buyer.seq.toNumber() / 8)
                ];
