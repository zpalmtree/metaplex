...
programCommand('upload')
    ...
    .action(async (files: string[], options, cmd) => {
        ...
        if (!(storage === 'arweave' || storage === 'ipfs')) {
            throw new Error(
                "Storage option must either be 'arweave' or 'ipfs'.",
            );
        }
        ...
        log.info(
            `ended at: ${new Date(
                endMs,
            ).toISOString()}. time taken: ${timeTaken}`,
        );
        if (warn) {
...
export async function doVerify(key, thisSlice, cacheContent) {
    ...
    if (!name.match(cacheItem.name) || !uri.match(cacheItem.link)) {
        ...
        if (cacheItem.verified) {
            log.info('Skipping already verified item');
            return true;
        }
...
programCommand('verify').action(async (directory, cmd) => {
    ...
    for (let i = 0; i < keys.length / BATCH_SIZE; i++) {
        const itemsRemaining = Math.min(
            BATCH_SIZE,
            keys.length - i * BATCH_SIZE,
        );
        ...
        for (let j = 0; j < itemsRemaining; j++) {
            ...
            console.log(
                `[${new String(item + 1).padStart(5, ' ')} / ${keys.length}]`,
            );
            ...
            const thisSlice = config.data.slice(
                ...
                CONFIG_ARRAY_START + 4 + CONFIG_LINE_SIZE * (item + 1),
            );

            processing.push(doVerify(key, thisSlice, cacheContent));
        }
...
programCommand('verify_price')
    ...
    .action(async (directory, cmd) => {
        ...
        if (lamports != candyMachineLamports) {
            throw new Error(
                `Expected price and CandyMachine's price do not match!`,
            );
        }
